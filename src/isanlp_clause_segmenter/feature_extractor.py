...
class FeatureExtractor:
    def __init__(self):
        self.categorical_features = [
            'lemma', 'upos', 'animacy', 'aspect', 'case', 'degree', 'gender', 'number', 'person', 'tense',
            'verbform', 'voice', 'deprel']

        self.categorical_features += ['parent_' + feature_name for feature_name in self.categorical_features]

        self.other_features = ['word_index', 'parent_index', 'is_capitalized', 'is_upper', 'n_children']
        ...
        self.out_features = ['lemma', 'upos', 'animacy', 'aspect', 'case', 'degree', 'gender', 'number', 'person',
                             'tense', 'verbform', 'voice', 'deprel', 'parent_lemma', 'parent_upos', 'parent_animacy',
                             'parent_aspect', 'parent_case', 'parent_degree', 'parent_gender', 'parent_number',
                             'parent_person', 'parent_tense', 'parent_verbform', 'parent_voice', 'parent_deprel',
                             'lemma_prev_1', 'upos_prev_1', 'animacy_prev_1', 'aspect_prev_1', 'case_prev_1',
                             'degree_prev_1', 'gender_prev_1', 'number_prev_1', 'person_prev_1', 'tense_prev_1',
                             'verbform_prev_1', 'voice_prev_1', 'deprel_prev_1', 'parent_lemma_prev_1',
                             'parent_upos_prev_1', 'parent_animacy_prev_1', 'parent_aspect_prev_1',
                             'parent_case_prev_1', 'parent_degree_prev_1', 'parent_gender_prev_1',
                             'parent_number_prev_1', 'parent_person_prev_1', 'parent_tense_prev_1',
                             'parent_verbform_prev_1', 'parent_voice_prev_1', 'parent_deprel_prev_1', 'lemma_prev_2',
                             'upos_prev_2', 'animacy_prev_2', 'aspect_prev_2', 'case_prev_2', 'degree_prev_2',
                             'gender_prev_2', 'number_prev_2', 'person_prev_2', 'tense_prev_2', 'verbform_prev_2',
                             'voice_prev_2', 'deprel_prev_2', 'parent_lemma_prev_2', 'parent_upos_prev_2',
                             'parent_animacy_prev_2', 'parent_aspect_prev_2', 'parent_case_prev_2',
                             'parent_degree_prev_2', 'parent_gender_prev_2', 'parent_number_prev_2',
                             'parent_person_prev_2', 'parent_tense_prev_2', 'parent_verbform_prev_2',
                             'parent_voice_prev_2', 'parent_deprel_prev_2', 'is_capitalized', 'is_upper', 'position',
                             'distance_to_end', 'n_children', 'is_capitalized_prev_1', 'is_upper_prev_1',
                             'n_children_prev_1', 'is_capitalized_prev_2', 'is_upper_prev_2', 'n_children_prev_2']
    ...
    def to_dataframe(self, conll_data: list):
        ...
        for sentence_id, sentence in enumerate(conll_data):
            childer_counter = collections.Counter(word_data['head'] for word_data in sentence)
            ...
            for word_index, word_data in enumerate(sentence):
                try:
                    features = {
                        ...
                        'position': word_index / (len(sentence) - 1) if len(sentence) > 1 else 0,
                        'distance_to_end': len(sentence) - word_index - 1,
                        'n_children': childer_counter[word_index],
                        'begin_segment': 0,
                        'parent_index': word_data['head'],
                        'parent_lemma': sentence[word_data['head'] - 1]['lemma'] if word_data['head'] > -1 else 'NA',
                        'parent_upos': sentence[word_data['head'] - 1]['upostag'] if word_data['head'] > -1 else 'NA',
                    ...
                    if word_data['head'] > -1:
                        ...
                        if parent_data['feats'] is not None:
                            for key in parent_data['feats']:
                                features['parent_' + key.lower()] = parent_data['feats'][key]
        ...
        for row in dataframe.itertuples():
            ...
            for i in range(1, n + 1):
                ...
                for feature in all_features:
                    expanded_dataframe.at[row.Index, prev_feature_name(feature, i)] = \
                        expanded_dataframe.at[(sentence_id, word_index - i), feature]

        return expanded_dataframe

    def _func_previous(self, d):
        return FeatureExtractor._add_previous_word_features(d, 2, self.categorical_features, self.other_features)
    ...
    def _lowest_common_ancestor(sentence_data, word_index1, word_index2):
        ...
        ancestors2 = {ancestor: distance for distance, ancestor in enumerate(ancestors2)}
    ...
    @staticmethod
    def _add_ancestor_features(dataframe, n, ancestor_categorical_features, ancestor_other_features, conll_data):
        def prev_feature_name(feature_name, k):
        ...
        for row in dataframe.itertuples():
            ...
            for i in range(n, 0, -1):
                ...
                ancestor, distance1, distance2 = FeatureExtractor._lowest_common_ancestor(
                ...
                if ancestor:
                    ...
                    for feature in all_features:
                        dataframe.at[row.Index, ancestor_feature_name(feature, i, i - 1)] = \
                            dataframe.at[(sentence_id, ancestor), feature]
                    dataframe.at[row.Index, ancestor_feature_name('distance', i, i - 1, i)] = distance1
                    dataframe.at[row.Index, ancestor_feature_name('distance', i, i - 1, i - 1)] = distance2
                    dataframe.at[row.Index, ancestor_feature_name('index', i, i - 1)] = ancestor

        return dataframe

    def _func_anc(self, d):
        features = FeatureExtractor._add_ancestor_features(d, 2,
                                                           self.ancestor_categorical_features,
    ...
    def _all_features(self, d):
        features = FeatureExtractor._add_previous_word_features(d, 2, self.categorical_features, self.other_features)
        # features = FeatureExtractor._add_ancestor_features(features, 2,
        #                                                    self.ancestor_categorical_features,
        #                                                    self.ancestor_other_features,
        #                                                    self.sentences)
